...
const signedYen = (v: number) => `${v >= 0 ? "+" : "−"}¥${Math.abs(v).toLocaleString()}`;
...
export default function SalesReferralFees() {
  ...
  const handleAddAdjustment = async () => {
    ...
    try {
      const { error } = await supabase.from("referral_fee_adjustments").insert({
        ...
        reason,
        amount: adjustmentMode === "add" ? adjustmentAmount : -adjustmentAmount,
        created_by: user?.id ?? null,
      });
      if (error) throw error;
      setAdjustmentOpen(false);
      await fetchData();
      toast.success("繰越・調整項目を追加しました");
    } catch (e: unknown) {
      toast.error(e instanceof Error ? e.message : "繰越・調整項目の追加に失敗しました");
    } finally {
  ...
  const handleDeleteAdjustment = async (adjustment: Adjustment) => {
    if (!window.confirm(`「${adjustment.reason}」${signedYen(adjustment.amount)}を削除しますか？`)) return;
    const { error } = await supabase
      ...
      .eq("store_id", storeId);
    if (error) {
      toast.error("繰越・調整項目の削除に失敗しました");
      return;
    }
    await fetchData();
    toast.success("繰越・調整項目を削除しました");
  };
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-[60px] md:ml-[240px] p-6">
        <div className="max-w-5xl mx-auto">
            ...
            <div className="flex items-center justify-between gap-3 mb-4 flex-wrap">
              ...
              <div className="flex items-center gap-2">
                <Button variant="outline" onClick={openAdjustmentDialog} disabled={rewardOptions.length === 0}>
                  <PlusCircle size={15} className="mr-1.5" />繰越・調整を追加
                </Button>
                {hasData && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <Card>
              <CardHeader className="pb-2"><CardTitle className="text-sm font-medium">支払合計</CardTitle></CardHeader>
              <CardContent>
                <div className={`text-2xl font-bold ${totalFees < 0 ? "text-red-600" : "text-primary"}`}>{yen(totalFees)}</div>
                {totalAdjustments !== 0 && (
                  <p className="text-xs text-muted-foreground mt-1">通常 {yen(grossFees)} ／ 調整 {signedYen(totalAdjustments)}</p>
                )}
              </CardContent>
            </Card>
            <Card>
                  ...
                  <table className="w-full min-w-[900px] text-sm">
                    <thead>
                      <tr className="border-b bg-muted/30 text-xs text-muted-foreground">
                        ...
                        <th className="text-right px-4 py-2.5 font-semibold">繰越・調整</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y">
                      ...
                      {displayedAdjustments.map((a) => {
                        const isAddition = a.amount > 0;
                        return (
                          <tr key={`adjustment-${a.id}`} className={isAddition ? "bg-emerald-50/60 hover:bg-emerald-50 transition-colors" : "bg-red-50/60 hover:bg-red-50 transition-colors"}>
                            <td className="px-4 py-2.5">
                              <div className="flex items-center gap-2 min-w-[160px]">
                                {isAddition ? <PlusCircle size={14} className="text-emerald-600 shrink-0" /> : <MinusCircle size={14} className="text-red-600 shrink-0" />}
                                <div className="min-w-0 flex-1">
                                  <p className={`font-medium break-words ${isAddition ? "text-emerald-700" : "text-red-700"}`}>{a.reason}</p>
                                  <p className="text-[11px] text-muted-foreground">{isAddition ? "繰越・加算" : "相殺・差引"}</p>
                                </div>
                                <button
                                  type="button"
                            ...
                            <td className="px-4 py-2.5 text-right text-muted-foreground">—</td>
                            <td className="px-4 py-2.5 text-right text-muted-foreground">—</td>
                            <td className={`px-4 py-2.5 text-right tabular-nums font-bold ${isAddition ? "text-emerald-700" : "text-red-600"}`}>{signedYen(a.amount)}</td>
                          </tr>
                        );
                      })}
                    </tbody>
                    <tfoot>
                      <tr className="border-t bg-muted/30 font-bold">
                        <td className="px-4 py-2.5 text-xs" colSpan={2}>小計</td>
                        ...
                        <td className="px-4 py-2.5 text-right tabular-nums text-xs text-primary">{yen(grossFees)}</td>
                        <td className={`px-4 py-2.5 text-right tabular-nums text-xs ${totalAdjustments < 0 ? "text-red-600" : "text-emerald-700"}`}>{totalAdjustments === 0 ? "—" : signedYen(totalAdjustments)}</td>
                      </tr>
                      <tr className="border-t bg-primary/5 font-bold">
                        <td className="px-4 py-3 text-xs text-right" colSpan={6}>支払合計</td>
                        <td className={`px-4 py-3 text-right tabular-nums ${totalFees < 0 ? "text-red-600" : "text-primary"}`}>{yen(totalFees)}</td>
                      </tr>
                    </tfoot>
                  </table>
                </div>
      ...
      <Dialog open={adjustmentOpen} onOpenChange={setAdjustmentOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>繰越・調整項目を追加</DialogTitle>
            <DialogDescription>
              {format(selectedMonth, "yyyy年M月", { locale: ja })}の支払額に、繰越分の加算または相殺分の差引を反映します。
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 mt-2">
            ...
            <div className="space-y-1.5">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                >
                  <PlusCircle size={15} className="mr-1.5" />加算・繰越支払
                </Button>
                <Button
                  type="button"
                  variant={adjustmentMode === "subtract" ? "destructive" : "outline"}
                  onClick={() => setAdjustmentMode("subtract")}
                  className="h-auto py-2.5"
                >
                  <MinusCircle size={15} className="mr-1.5" />相殺・差引
                </Button>
              </div>
            </div>
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder={adjustmentMode === "add" ? "例：7月からの繰越分" : "例：店落ち分の相殺"}
                maxLength={120}
              />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="adjustment-amount">金額</Label>
              <Input
                ...
                placeholder="0"
              />
              <p className="text-xs text-muted-foreground">金額は正数で入力してください。選んだ反映方法に応じてプラス・マイナスを付けます。</p>
            </div>
            <Button onClick={handleAddAdjustment} disabled={savingAdjustment} className="w-full">
              ...
              {savingAdjustment ? "追加中..." : adjustmentMode === "add" ? "支払額に加算する" : "支払額から差し引く"}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
