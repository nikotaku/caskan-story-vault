-- O2を認証情報の正とし、魂セラピストの初回ログインにも同じID・PWを使う。
-- O2登録メールアドレスは認証情報とは分けて casts に保持する。

update public.casts c
set o2_login_email = source.login_email
from (
  select profile.cast_id, max(trim(profile.soul_account_email)) as login_email
  from public.external_cast_profiles profile
  where profile.provider = 'estama'
    and nullif(trim(profile.soul_account_email), '') is not null
    and trim(profile.soul_account_email) ~* '^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$'
  group by profile.cast_id
) source
where c.id = source.cast_id
  and nullif(trim(c.o2_login_email), '') is null;
...
create or replace function public.get_sns_connection_overview_v4(p_store_id uuid)
returns table(
  cast_id uuid,
  cast_name text,
  photo text,
...
create or replace function public.save_sns_connection_admin_v4(
  p_store_id uuid,
  p_cast_id uuid,
  p_login_id text,
  p_password text,
...
begin
  ...
  if length(coalesce(v_login_id, '')) > 255
     or length(coalesce(v_x_login_id, '')) > 255 then
    raise exception 'IDは255文字以内で入力してください';
  end if;
  if length(coalesce(v_password, '')) > 512
     or length(coalesce(v_x_password, '')) > 512 then
  ...
  v_x_profile_url := case
    when v_x_login_id is null then null
    else 'https://x.com/' || v_x_login_id
  end;
  ...
  if v_x_login_id is null then
    ...
    insert into public.cast_site_credentials (
      ...
      p_cast_id, p_store_id, 'x', v_x_login_id, v_x_password, now()
    );
  end if;
end;
$$;
...
begin
  ...
  insert into public.cast_site_credentials (
    ...
    v_cast_id, v_store_id, 'esutama', v_login_id, v_password, now()
  )
  on conflict (cast_id, site) do update
  set store_id = excluded.store_id,
      login_id = excluded.login_id,
...
revoke all on function public.get_sns_connection_overview_v4(uuid) from public, anon;
revoke all on function public.save_sns_connection_admin_v4(uuid, uuid, text, text, text, text, text, text, boolean, boolean) from public, anon;
revoke all on function public.save_therapist_o2_credentials(text, text, text, text) from public;

grant execute on function public.get_sns_connection_overview_v4(uuid) to authenticated;
grant execute on function public.save_sns_connection_admin_v4(uuid, uuid, text, text, text, text, text, text, boolean, boolean) to authenticated;
grant execute on function public.save_therapist_o2_credentials(text, text, text, text) to anon, authenticated;
