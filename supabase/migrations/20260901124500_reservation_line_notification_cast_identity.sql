-- Preserve therapist visit history across the legacy-store to Enka identity migration.
-- A real-name match is preferred; the public cast name is used only when either
-- side has no real name. Identity fields are used for matching and never returned.
