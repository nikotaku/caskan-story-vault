-- セラピストへの予約共有LINEでクーポン種別を判定できるように、
-- get_reservation_line_context の返却値に discount_names（適用された割引名の配列）を追加する。
-- エスたま限定1万円クーポン（総額10,000円クーポン）適用時に、
-- 口コミ確認で5,000円キャッシュバックの案内をメッセージへ追記するために使用する。
-- ※本番DBの最新定義（cast_identity 版）をベースにしているため、既存の履歴ロジックは維持される。
...
AS $function$
  with target as materialized (
    select
      ...
      reservation.discount_ids,
      public.norm_phone(reservation.customer_phone) as phone_key,
      cast_record.name as cast_name,
      nullif(lower(btrim(cast_record.name)), '') as cast_name_key,
      nullif(lower(btrim(cast_record.real_name)), '') as cast_real_name_key,
  ...
  select jsonb_build_object(
    ...
    'discount_names', coalesce((
      select jsonb_agg(discount_record.name order by discount_record.name)
      from public.discounts as discount_record
      where discount_record.id::text = any(coalesce(target.discount_ids, array[]::text[]))
    ), '[]'::jsonb),
    ...
    'cast_visit_count', case
      when length(target.phone_key) < 10
        ...
        where history.store_id = any(target.visit_store_ids)
          and exists (
            select 1
            from public.casts as history_cast
            where history_cast.id = history.cast_id
              and (
                history_cast.id = target.cast_id
                or nullif(lower(btrim(history_cast.real_name)), '') = target.cast_real_name_key
                or (
                  (target.cast_real_name_key is null
                    or nullif(lower(btrim(history_cast.real_name)), '') is null)
                  and nullif(lower(btrim(history_cast.name)), '') = target.cast_name_key
                )
              )
          )
          and public.norm_phone(history.customer_phone) = target.phone_key
